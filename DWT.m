...
% Apply the DWT to the image channels
[LL1, LH1, HL1, HH1] = dwt2(x1, 'haar');
[LL2, LH2, HL2, HH2] = dwt2(x2, 'haar');
[LL3, LH3, HL3, HH3] = dwt2(x3, 'haar');

% Resize the mask to match the size of the DWT result
a_resized = imresize(a, size(LH1));

% Coefficient of watermark strength
g = 0.5; % Chose this value after trial and error

% Apply the watermark and the IDWT
LH1(logical(a_resized)) = LH1(logical(a_resized)) + g * watermark_red(logical(a_resized));
LH2(logical(a_resized)) = LH2(logical(a_resized)) + g * watermark_green(logical(a_resized));
LH3(logical(a_resized)) = LH3(logical(a_resized)) + g * watermark_blue(logical(a_resized));

% Apply the IDWT to get the watermarked image
y1 = idwt2(LL1, LH1, HL1, HH1, 'haar');
y2 = idwt2(LL2, LH2, HL2, HH2, 'haar');
y3 = idwt2(LL3, LH3, HL3, HH3, 'haar');
